...
import { LinkageDefaultError } from "../models/errors/linkagedefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | LinkageDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | LinkageDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new LinkageDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new LinkageDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
