...
* [getOverlays](#getoverlays) - use to set the summary
...
async function run() {
  const result = await linkage.nodes.getOverlays();
...
import { nodesGetOverlays } from "@linkage-open/sdk/funcs/nodesGetOverlays.js";
...
async function run() {
  const res = await nodesGetOverlays(linkage);
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("nodesGetOverlays failed:", res.error);
  }
}
